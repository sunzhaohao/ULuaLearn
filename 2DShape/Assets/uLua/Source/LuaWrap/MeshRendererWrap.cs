...
public class MeshRendererWrap
{
	public static void Register(IntPtr L)
	{
		...
		LuaField[] fields = new LuaField[]
		{
			...
			new LuaField("subMeshStartIndex", get_subMeshStartIndex, null),
		};
	...
	static int get_subMeshStartIndex(IntPtr L)
	{
		...
		if (obj == null)
		{
			...
			if (types == LuaTypes.LUA_TTABLE)
			{
				LuaDLL.luaL_error(L, "unknown member name subMeshStartIndex");
			}
			else
			{
				LuaDLL.luaL_error(L, "attempt to index subMeshStartIndex on a nil value");
			}
		}

		LuaScriptMgr.Push(L, obj.subMeshStartIndex);
		return 1;
	}
